...
def check_args_input(args):
	...
	to adhere to boundary conditions.

	Also print warnings for input that would
	seem abnormal.
	...
	assert args.colors[0] < args.colors[1], "Functionality may be compromised if the first color value > the second."
...
def gen_prob_mass_fcn(args, star_mags):
	...
	can be evaluated by simply evaluating the
	model luminosity function over the input magnitude 
	range. This relative abundance,once normalized, 
	is the probability mass function (PMF).
	
	Inputs: Command-line arguments, list of all possible star magnitudes
	...
	# a uniform distribution that can later be crafted (after photometery)
	# to a luminosity function
	
	# Below the function's exponential is scaled down to avoid 
	# any possible floating point errors if the magnitude 
	# range is massive, for example
...
def gen_star_color(args, star_mag):
	"""
	Once the magnitude of the star in the primary
	filter is set, we can now give it a randomly
	chosen color based on the user input	
	"""
	...
	# line: star_mag - args.trgbmag = args.cslope[0] * ( mid_star_color  - mid_color )

	mid_color = 0.5 * ( args.colors[0] + args.colors[1] )

	mid_star_color = mid_color + (star_mag - args.trgbmag)/args.cslope 

	# For convenience+clarity, define variable for minmax color
